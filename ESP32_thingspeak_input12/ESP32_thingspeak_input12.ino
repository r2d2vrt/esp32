...
// ============================================================================
// ВХОДЫ - СТРОГИЙ ПОРЯДОК: IN0, IN1, IN2, IN3
// ============================================================================
const int numInputs = 4;
const int inputPins[numInputs] = {IN0, IN1, IN2, IN3};  // Порядок: 0=IN0, 1=IN1, 2=IN2, 3=IN3

// Состояния входов (индекс соответствует номеру входа)
int inputStates[4];           // Текущие состояния: [0]=IN0, [1]=IN1, [2]=IN2, [3]=IN3
int lastDebounceStates[4];
int lastSteadyStates[4];
int previousSteadyStates[4];
unsigned long lastDebounceTimes[4];

// Направление изменения для каждого входа: true = подъем (0→1), false = спад (1→0)
bool inputRisingEdge[4];      // [0]=IN0, [1]=IN1, [2]=IN2, [3]=IN3

// Строка состояния входов: "IN0_IN1_IN2_IN3" (например "1000")
String currentInputStatus = "0000";
...
bool alarmActivateOutputs = false;  // Флаг: активировать выходы или нет
...
void setup() {
  ...
  for (int i = 0; i < numInputs; i++) {
    ...
    inputStates[i] = val;
    lastDebounceStates[i] = val;
    lastSteadyStates[i] = val;
    previousSteadyStates[i] = val;
    lastDebounceTimes[i] = 0;
    inputRisingEdge[i] = false;
  }
  
  // Формирование начальной строки состояния входов
  // Порядок: IN0 (позиция 0), IN1 (позиция 1), IN2 (позиция 2), IN3 (позиция 3)
  currentInputStatus = String(inputStates[0]) + String(inputStates[1]) + 
                       String(inputStates[2]) + String(inputStates[3]);
  ...
  Serial.println("inputPins[0] = GPIO" + String(inputPins[0]) + " (IN0)");
  Serial.println("inputPins[1] = GPIO" + String(inputPins[1]) + " (IN1)");
  Serial.println("inputPins[2] = GPIO" + String(inputPins[2]) + " (IN2)");
  Serial.println("inputPins[3] = GPIO" + String(inputPins[3]) + " (IN3)");
  Serial.println("Initial input states:");
  Serial.println("  IN0 (index 0): " + String(inputStates[0]));
  Serial.println("  IN1 (index 1): " + String(inputStates[1]));
  Serial.println("  IN2 (index 2): " + String(inputStates[2]));
  Serial.println("  IN3 (index 3): " + String(inputStates[3]));
  Serial.println("Initial status string: " + currentInputStatus);
  Serial.println("=====================");
...
bool readInputs(unsigned long currentMillis) {
  ...
  for (int i = 0; i < numInputs; i++) {
    ...
    if ((currentMillis - lastDebounceTimes[i]) >= DEBOUNCE_DELAY) {
      if (reading != lastSteadyStates[i]) {
        ...
        if (lastSteadyStates[i] == LOW && reading == HIGH) {
          inputRisingEdge[i] = true;   // Подъем (0→1)
        } else {
          inputRisingEdge[i] = false;  // Спад (1→0)
        }
        
        lastSteadyStates[i] = reading;
        inputStates[i] = reading;  // Обновляем массив состояний
        changed = true;
      }
    }
  }
  ...
  if (changed) {
    // Формируем строку в СТРОГОМ порядке: IN0, IN1, IN2, IN3
    // Позиция 0 = IN0, Позиция 1 = IN1, Позиция 2 = IN2, Позиция 3 = IN3
    currentInputStatus = String(inputStates[0]) + String(inputStates[1]) + 
                         String(inputStates[2]) + String(inputStates[3]);
    
    Serial.println("=== INPUTS CHANGED ===");
    Serial.println("IN0 (pos 0): " + String(inputStates[0]) + " " + (inputRisingEdge[0] ? "RISING" : "FALLING"));
    Serial.println("IN1 (pos 1): " + String(inputStates[1]) + " " + (inputRisingEdge[1] ? "RISING" : "FALLING"));
    Serial.println("IN2 (pos 2): " + String(inputStates[2]) + " " + (inputRisingEdge[2] ? "RISING" : "FALLING"));
    Serial.println("IN3 (pos 3): " + String(inputStates[3]) + " " + (inputRisingEdge[3] ? "RISING" : "FALLING"));
    Serial.println("Status string: " + currentInputStatus);
    Serial.println("======================");
  }
...
void enterAlarm(unsigned long currentMillis) {
  ...
  for (int i = 0; i < numInputs; i++) {
    if (lastSteadyStates[i] != previousSteadyStates[i]) {
      ...
      Serial.println("Changed input index: " + String(i) + " (IN" + String(i) + ") " + 
                     (inputRisingEdge[i] ? "RISING" : "FALLING"));
    }
  }
  
  Serial.println("ALARM: " + String(numChanged) + " input(s) changed");
  Serial.println("Current status string: " + currentInputStatus);
  
  // Инициализация тревоги
  alarmStartTime = currentMillis;
  // При отправке на сервер добавляем "1" перед строкой состояния
  alarmMessage = "1" + currentInputStatus;
  alarmMessageDelivered = false;
  alarmLastSendTime = 0; // Немедленная первая отправка
  
  Serial.println("Alarm message to send: " + alarmMessage);
  
  if (numChanged == 1) {
    // Одиночное изменение
    int changedInput = changedInputs[0];
    bool isRising = inputRisingEdge[changedInput];
    ...
    if (changedInput == 0) {
      ...
      if (isRising) {
        // Подъем (0→1): OUT1 сразу, OUT0 через 15 сек
        alarmActivateOutputs = true;
        ...
        Serial.println("Alarm type: IN0 RISING - activate outputs");
      } else {
        // Спад (1→0): только отправка сообщения
        alarmActivateOutputs = false;
        alarmOut1Activated = false;
        alarmOut0Activated = false;
        Serial.println("Alarm type: IN0 FALLING - no outputs");
      }
      ...
      if (isRising) {
        // Подъем (0→1): OUT0 и OUT1 сразу
        alarmActivateOutputs = true;
        alarmOut0ActivateTime = currentMillis;
        alarmOut0Activated = false;
        alarmOut1ActivateTime = currentMillis;
        alarmOut1Activated = false;
        Serial.println("Alarm type: IN1 RISING - activate outputs");
      } else {
        // Спад (1→0): только отправка сообщения
        alarmActivateOutputs = false;
        alarmOut0Activated = false;
        alarmOut1Activated = false;
        Serial.println("Alarm type: IN1 FALLING - no outputs");
      }
      
    } else {
      // IN2 или IN3 изменился: без активации выходов
      currentState = STATE_ALARM_IN23;
      alarmActivateOutputs = false;
      Serial.println("Alarm type: IN" + String(changedInput) + " (index " + String(changedInput) + ")");
    }
...
void handleAlarmIN0(unsigned long currentMillis) {
  ...
  // Управление выходами (только если активация разрешена)
  if (alarmActivateOutputs) {
    // OUT1 - немедленно
  ...
  if (secLeft < 0) secLeft = 0;
  displayAlarmIN0(currentInputStatus, alarmMessageDelivered, secLeft, ALARM_OUT_DURATION / 1000, alarmActivateOutputs);
...
void handleAlarmIN1(unsigned long currentMillis) {
  ...
  int secLeft = (ALARM_OUT_DURATION - elapsed) / 1000;
  if (secLeft < 0) secLeft = 0;
  displayAlarmIN1(currentInputStatus, alarmMessageDelivered, secLeft, ALARM_OUT_DURATION / 1000, alarmActivateOutputs);
...
bool sendMainData() {
  ...
  // Поле 1: текущее состояние входов в формате "IN0_IN1_IN2_IN3"
  ...
  Serial.println("Field 1 (inputs): " + currentInputStatus);
...
bool sendAlarmMessage(String msg) {
  ...
  HTTPClient http;
  // Сообщение уже содержит "1" в начале: "1" + currentInputStatus
...
void displayMain(int secLeft, int maxPeriod) {
  ...
  // Строка 4: Состояние входов (IN0_IN1_IN2_IN3)
...
void displayAlarmIN0(String inputs, bool delivered, int secLeft, int maxPeriod, bool activateOut) {
  display.clearDisplay();
  display.setTextColor(SSD1306_WHITE);
...
void displayAlarmIN1(String inputs, bool delivered, int secLeft, int maxPeriod, bool activateOut) {
  display.clearDisplay();
  display.setTextColor(SSD1306_WHITE);
